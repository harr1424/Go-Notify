...
func sendPushNotification(targetToken string, location string, time string, value string) {
	...
	alertTitle := fmt.Sprintf("Frost Alert for %s", location)
	alertSubtitle := fmt.Sprintf("A temperature of %s is expected on %s", newAlert.Value, newAlert.Time)
	payload := PAYLOAD.NewPayload().Alert(alertTitle).AlertSubtitle(alertSubtitle)
